...
//! appear as `blueprint.plan_publish`, `blueprint.plan_status`, … — which is exactly
//! the `<server>.<tool>` id a workflow's `mcp` node takes.
//!
//! **Tool names register bare.** The `blueprint.` prefix is Core's, formed from the
//! `mcp_servers` key; a tool that names itself `blueprint.plan_publish` here would be
//! published as `blueprint.blueprint.plan_publish` and never be called by anything.
//!
...
mod tests {
    ...
    fn the_four_contracted_tools_are_declared_bare_with_usable_schemas() {
        ...
        for tool in tools {
            ...
            assert!(
                !name.starts_with("blueprint."),
                "{name} self-prefixes — Core adds `blueprint.`, so this would register \
				as blueprint.blueprint.{name} and never be called"
            );
            assert!(
