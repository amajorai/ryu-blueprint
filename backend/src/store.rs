...
mod tests {
    use super::*;
    use crate::model::{
        AnnKind, Block, BlockKind, Decision, PlanStatus, Step, StepStatus, Target,
    };
    ...
    fn a_plan_round_trips_with_its_revisions() {
        ...
        assert_eq!(store.get_plan("deploy").expect("gets").expect("some").title, "Plan deploy");
        assert_eq!(
            store.get_revision("deploy", 2).expect("gets").expect("some").revision,
            2
        );
    ...
    fn every_entry_point_refuses_an_id_that_could_escape_the_store() {
        ...
        for bad in [
            ...
            assert!(store.delete_plan(bad).is_err(), "delete_plan accepted '{bad}'");
            assert!(store.list_revisions(bad).is_err(), "list_revisions accepted '{bad}'");
            assert!(store.annotations(bad).is_err(), "annotations accepted '{bad}'");
    ...
    fn a_verdict_is_stored_per_revision_so_a_revise_does_not_inherit_an_approval() {
        ...
        assert_eq!(
            store.verdict_for("p", 1).expect("gets").expect("some").verdict,
            Decision::Approved
        );
        assert!(
