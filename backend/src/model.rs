...
pub struct StepInput {
    /// Stable id for this step, which other steps reference in their `depends_on`.
    /// Omit it and one is derived from the title.
    #[serde(default)]
    pub id: Option<String>,
    /// One line naming the step, as it appears on the graph node.
    pub title: String,
    /// A sentence or two of detail shown when the reviewer opens the step.
    #[serde(default)]
    pub summary: Option<String>,
    /// Ids of the steps that must finish before this one starts. These are the edges
    /// of the graph the reviewer reads, so a missing edge shows the work as parallel
    /// when it is not. A cycle or a dangling id is refused before anything is stored.
    #[serde(default)]
    pub depends_on: Vec<String>,
    /// Files this step is expected to touch.
    #[serde(default)]
    pub files: Vec<String>,
    /// What could go wrong here, in the author's own words. Never inferred — a step
    /// with no stated risk is shown as having none.
    #[serde(default)]
    pub risk: Option<String>,
}
