...
";
    ...
    fn a_block_id_survives_an_edit_to_a_different_block() {
        ...
        assert_eq!(
            first.blocks[1].id, second.blocks[1].id,
            "untouched para moved"
        );
        assert_ne!(
    ...
    fn a_block_id_survives_being_moved_and_having_neighbours_inserted() {
        ...
        assert_eq!(
            ...
            "content addressing must ignore position"
        );
    ...
    fn numbered_items_become_steps_with_linear_dependencies() {
        ...
        assert_eq!(
            parsed.blocks[1].step_id.as_deref(),
            Some("s_migrate-schema")
        );
    }
    ...
    fn after_none_makes_a_root_and_an_unresolvable_ref_is_dropped_not_fatal() {
        ...
        assert!(
            parsed.steps[1].depends_on.is_empty(),
            "`after: none` means root"
        );
        assert!(
    ...
    fn the_files_convention_is_collected_from_both_forms() {
        ...
        assert_eq!(
            parsed.steps[1].summary, None,
            "a files: line is not a summary"
        );
    }
...
";
        ...
        assert_eq!(
            parsed.steps[0].summary.as_deref(),
            Some("We create the tables first.")
        );
        assert_eq!(parsed.steps[1].depends_on, vec!["s_set-up"]);
