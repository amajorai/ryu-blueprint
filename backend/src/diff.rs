...
pub fn diff_blocks(from: &[Block], to: &[Block]) -> Vec<BlockDiff> {
    ...
    for ti in 0..to.len() {
        ...
        let candidate = (search_from..from.len())
            .find(|fi| !consumed[*fi] && pairable(&from[*fi], &to[ti]));
...
mod tests {
    ...
    fn pairing_never_runs_backwards() {
        ...
        let diff = diff_blocks(&a.blocks, &b.blocks);
        assert_eq!(diff[0].previous.as_ref().expect("previous").text, "First old.");
        assert_eq!(diff[1].previous.as_ref().expect("previous").text, "Second old.");
    }
