...
/// (`/api/blueprint/...`, `{id}` in brace form) while the router above registers paths
/// relative to the mount in axum's `:id` form. The two forms differ ON PURPOSE — Core
/// nests this router at `/api/blueprint`, and the document has to describe the URL a
/// caller actually hits. Do not "align" either side.
///
/// This app also ships a stdio `mcp_servers` server ([`crate::mcp`]) whose four tools
/// — `plan_publish`, `plan_status`, `plan_get`, `step_update` — are the same operations
/// as `POST /plans`, `GET /plans/{id}/feedback`, `GET /plans/{id}` and
/// `POST /plans/{id}/steps/{step_id}`. That overlap is deliberate and harmless: both
/// surfaces call the SAME functions ([`publish`], [`feedback_for`], [`plan_detail`],
/// [`update_step`]), so they cannot drift, and a node that has the MCP server disabled
/// still gets the whole surface as derived tools. The six routes with no MCP twin
/// (revisions, diff, annotations, verdict, delete) exist ONLY here.
#[must_use]
pub fn openapi() -> utoipa::openapi::OpenApi {
    <BlueprintApiDoc as utoipa::OpenApi>::openapi()
}
...
///
/// `StepInput`, `Target`, `AnnKind`, `Decision` and `StepStatus` are here because each
/// is reachable only TRANSITIVELY, through one of the four request bodies — the
/// transitive graph is the part that breaks builds.
...
pub struct PublishRequest {
    ...
    /// title. Reusing an id is how a plan is REVISED after a reviewer asked for
    /// changes; omitting it publishes a separate plan instead.
    #[serde(default)]
    pub plan_id: Option<String>,
    /// One line naming the plan, shown in the review list and used to mint the id.
    pub title: String,
    /// The plan itself, as markdown. Headings, paragraphs, list items and fenced
    /// blocks each become separately annotatable, so a reviewer can object to one
    /// paragraph rather than to the whole document.
    pub markdown: String,
    /// The step graph, when the caller already knows it. Explicit steps WIN over
    /// derivation — an agent that knows its own DAG should not have it guessed at
    /// from prose. Omit this and the steps are derived from the markdown.
    // Inlined so the model reads the step fields (`title`, `depends_on`, …) instead
    // of a `$ref` it cannot follow: Core resolves refs only one level into a schema.
    #[serde(default)]
    #[schema(inline)]
    pub steps: Option<Vec<StepInput>>,
    /// A rendered artifact — a mock, a report — the reviewer looks at beside the plan.
    #[serde(default)]
    pub artifact_html: Option<String>,
    /// The conversation this plan came out of, so the review links back to it.
    #[serde(default)]
    pub conversation_id: Option<String>,
    /// Which agent authored it.
    #[serde(default)]
    pub agent_id: Option<String>,
    /// Free-form provenance shown in the list ("claude-code", "workflow:deploy").
    #[serde(default)]
...
#[utoipa::path(
    ...
    tag = "Blueprint",
    summary = "Publish a plan for a human to review, or revise one they asked for changes to. Returns the plan id to poll.",
...
#[utoipa::path(
    ...
    path = "/api/blueprint/plans",
    tag = "Blueprint",
    summary = "List published plans and their review status.",
    params(("status" = Option<String>, Query, description = "Keep only plans in this state: in_review, approved or changes_requested. Anything else is rejected rather than returning an empty list.")),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
async fn list_plans(
    State(ctx): State<Arc<Ctx>>,
...
#[utoipa::path(
    get,
    path = "/api/blueprint/plans/{id}",
    tag = "Blueprint",
    summary = "Read a whole plan in one call: its current revision, every annotation on it, the verdict if there is one, and the step layout.",
    params(("id" = String, Path, description = "Plan id, as returned when it was published")),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
async fn get_plan(
    State(ctx): State<Arc<Ctx>>,
...
#[utoipa::path(
    ...
    path = "/api/blueprint/plans/{id}",
    tag = "Blueprint",
    summary = "Delete a plan with all its revisions, annotations and verdicts. Irreversible.",
    params(("id" = String, Path, description = "Plan id to delete")),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
async fn delete_plan(
    ...
    let deleted = ctx.store.delete_plan(&id).map_err(ApiError::from_store)?;
    Ok(Json(json!({ "deleted": deleted })))
}

#[utoipa::path(
    get,
    path = "/api/blueprint/plans/{id}/revisions",
    tag = "Blueprint",
    summary = "List a plan's revisions, newest last, with the block and step count of each.",
    params(("id" = String, Path, description = "Plan id whose history to list")),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
#[utoipa::path(
    get,
    path = "/api/blueprint/plans/{id}/revisions/{rev}",
    tag = "Blueprint",
    summary = "Read one specific revision of a plan exactly as it was published, with its step layout.",
    params(
        ("id" = String, Path, description = "Plan id"),
        ("rev" = u32, Path, description = "Revision number, 1-based"),
    ),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
#[utoipa::path(
    get,
    path = "/api/blueprint/plans/{id}/diff",
    tag = "Blueprint",
    summary = "See what changed between two revisions of a plan, block by block.",
    params(
        ("id" = String, Path, description = "Plan id to diff"),
        ("from" = Option<u32>, Query, description = "Revision to compare against. Defaults to the one before `to`, so a bare call answers 'what changed in the latest revise?'."),
        ("to" = Option<u32>, Query, description = "Revision to compare. Defaults to the current one."),
    ),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
async fn diff_route(
...
struct AnnotationRequest {
    /// Which revision this is written against. Defaults to the current one. An
    /// annotation never migrates — it describes the text that existed when it was
    /// written.
    #[serde(default)]
    revision: Option<u32>,
    /// What the annotation is attached to: `{"type":"block","id":…}` for one piece of
    /// the document, `{"type":"step","id":…}` for one node of the graph, or
    /// `{"type":"plan"}` for the plan as a whole. An id that is not in that revision
    /// is refused rather than stored as an annotation that renders nowhere.
    // Inlined so the model sees the three variant shapes rather than a `$ref` it
    // cannot follow — Core resolves refs only one level into a schema.
    #[schema(inline)]
    target: Target,
    /// How loudly the author should hear this: `blocker` stops the work, `redline`
    /// proposes replacement text, `question` asks, `comment` notes.
    #[schema(inline)]
    kind: AnnKind,
    /// The note itself. Required unless this is a redline carrying a `suggestion`.
    body: String,
    /// The replacement text a redline proposes.
    #[serde(default)]
    suggestion: Option<String>,
    /// Who wrote it, shown beside the note.
    #[serde(default)]
...
#[utoipa::path(
    post,
    path = "/api/blueprint/plans/{id}/annotations",
    tag = "Blueprint",
    summary = "Attach a blocker, redline, question or comment to one block or step of a plan.",
    params(("id" = String, Path, description = "Plan id to annotate")),
...
async fn create_annotation(
    ...
    let revision = ctx
        ...
        .map_err(ApiError::from_store)?
        .ok_or_else(|| {
            ApiError::missing(format!("plan '{id}' has no revision {revision_number}"))
        })?;
...
#[utoipa::path(
    delete,
    path = "/api/blueprint/plans/{id}/annotations/{annotation_id}",
    tag = "Blueprint",
    summary = "Remove one annotation from a plan.",
    params(
        ("id" = String, Path, description = "Plan id the annotation is on"),
        ("annotation_id" = String, Path, description = "Id of the annotation to remove"),
    ),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
pub struct VerdictRequest {
    /// The decision about the plan's CURRENT revision: `approved` or
    /// `changes_requested`. Revising the plan afterwards resets it — an approval is a
    /// statement about one revision, never about the plan forever.
    // Inlined so the model reads the two literal values rather than a `$ref` it cannot
    // follow one level down.
    #[schema(inline)]
    pub verdict: Decision,
    /// A line explaining the decision, read back to the author with the verdict.
    #[serde(default)]
...
#[utoipa::path(
    post,
    path = "/api/blueprint/plans/{id}/verdict",
    tag = "Blueprint",
    summary = "Approve a plan's current revision or send it back for changes, with a note.",
    params(("id" = String, Path, description = "Plan id to decide on")),
    request_body = VerdictRequest,
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
pub struct StepUpdate {
    /// Where this step now stands: `todo`, `in_progress`, `done` or `blocked`.
    /// Re-sending the same value is allowed and changes nothing.
    // Inlined per the `Option<enum>` rule: without it the field renders as a nullable
    // wrapper burying the `$ref`, and the model sees no values it may send.
    #[serde(default)]
    #[schema(inline)]
...
#[utoipa::path(
    post,
    path = "/api/blueprint/plans/{id}/steps/{step_id}",
    tag = "Blueprint",
    summary = "Move one step of a plan as the work lands, so a reviewer watching the graph sees progress without asking.",
    params(
        ("id" = String, Path, description = "Plan id the step belongs to"),
        ("step_id" = String, Path, description = "Id of the step to move, as it appears in the plan's step list"),
    ),
    request_body = StepUpdate,
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
async fn step_route(
    State(ctx): State<Arc<Ctx>>,
...
#[utoipa::path(
    get,
    path = "/api/blueprint/plans/{id}/feedback",
    tag = "Blueprint",
    summary = "Check whether a plan has been decided, and read the reviewer's findings as text ready to act on. `pending` true means keep waiting, not approved.",
    params(("id" = String, Path, description = "Plan id to check")),
    responses((status = 200, description = "OK", body = serde_json::Value))
)]
...
mod tests {
    ...
    async fn publishing_twice_under_one_id_appends_a_revision_and_resets_the_status() {
        ...
        assert!(ctx
            .store
            .verdict_for("deploy-v2", 1)
            .expect("gets")
            .is_some());
        assert!(ctx
            .store
            .verdict_for("deploy-v2", 2)
            .expect("gets")
            .is_none());
    }
    ...
    async fn explicit_steps_are_attributed_to_the_prose_that_names_them_and_never_to_code() {
        ...
        assert_eq!(
            rev.blocks[1].step_id, None,
            "unrelated prose is not attributed"
        );
        assert_eq!(
    ...
    async fn an_empty_publish_is_refused_with_a_message_that_says_what_to_send() {
        ...
        let err = publish(&ctx, request("", "body"))
            .await
            .expect_err("refused");
    ...
    async fn the_bare_diff_of_a_brand_new_plan_reads_as_all_added() {
        ...
        assert!(
            blocks
                .iter()
                .all(|b| b.status == crate::diff::BlockChange::Added),
            "{blocks:?}"
        );
    }
    ...
    async fn the_bare_diff_of_a_revised_plan_compares_it_with_the_previous_revision() {
        ...
        assert!(same
            .iter()
            .all(|b| b.status == crate::diff::BlockChange::Same));
    ...
    async fn feedback_reports_pending_until_someone_decides() {
        ...
        assert!(
            decided.text.contains("CHANGES REQUESTED"),
            "{}",
            decided.text
        );
    ...
    async fn an_annotation_must_point_at_something_that_exists() {
        ...
        assert_eq!(
            feedback::pending_count(&ctx.store.annotations("anchor").expect("lists"), 1),
            1
        );
    }
    ...
    fn every_write_route_documents_a_typed_request_body() {
        ...
        // zero visible arguments: the model can see it and can never fill it in.
        let doc = serde_json::to_value(openapi()).expect("the document serializes");
        for (path, verb) in [
            ("/api/blueprint/plans", "post"),
            ("/api/blueprint/plans/{id}/annotations", "post"),
            ("/api/blueprint/plans/{id}/verdict", "post"),
            ("/api/blueprint/plans/{id}/steps/{step_id}", "post"),
        ] {
            let schema =
    ...
    #[test]
    fn the_enum_arguments_spell_out_their_values_instead_of_hiding_behind_a_ref() {
        // Core resolves a `$ref` only ONE level into a schema, so a property-level
        // reference reaches the model as an empty object and the argument becomes
        // unfillable. `#[schema(inline)]` on those fields is what prevents it.
        let doc = serde_json::to_value(openapi()).expect("the document serializes");
        let schemas = &doc["components"]["schemas"];

        let verdict = schemas["VerdictRequest"].to_string();
        assert!(
            verdict.contains("changes_requested"),
            "the verdict argument does not name its values: {verdict}"
        );

        let step = schemas["StepUpdate"].to_string();
        assert!(
            step.contains("in_progress"),
            "the step status argument does not name its values: {step}"
        );

        let annotation = schemas["AnnotationRequest"].to_string();
        assert!(
            annotation.contains("blocker") && annotation.contains("\"step\""),
            "the annotation body hides its kind or target shapes: {annotation}"
        );

        let publish = schemas["PublishRequest"].to_string();
        assert!(
            publish.contains("depends_on"),
            "the publish body hides the step fields, so a caller cannot supply a DAG: \
             {publish}"
        );
    }
