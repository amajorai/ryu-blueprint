...
## Formal graph contract

The sidecar's deterministic graph layer is covered by
`backend/fixtures/conformance.json`. The Rust conformance test pins exact layouts
and rejection errors, while the same valid layouts are projected into Ryu's pinned
Lean model, where a strictly decreasing layer proves that the dependency graph
cannot cycle. This covers the graph model, not the browser renderer or a live review.
...
The switches that are real: **enabling the app at all** (it is not pre-installed — in
`CORE_PLUGINS` for tier and never in `CORE_PREINSTALLED`, so a sidecar binary a normal
install does not have is never spawned uninvited), and the
...
Not pre-installed — `CORE_PLUGINS`, deliberately **not** `CORE_PREINSTALLED`, because it owns
an out-of-process binary an ordinary install does not have. A fresh machine has no
lifecycle record until the user explicitly installs it.
...
<a href="https://github.com/amajorai/ryu-blueprint/stargazers">
  <picture>
