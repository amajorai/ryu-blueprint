...
Blueprint for Ryu — visual plan review: an agent publishes its plan over MCP, a human reads it as addressable markdown blocks plus a dependency graph derived from the steps, annotates it inline (comment, redline, question, blocker), and approves or requests changes — and the agent reads the verdict back as structured text.
...
- Binary: `ryu-blueprint` from the [Ryu releases](https://github.com/amajorai/ryu/releases).
- Crate: `cargo install ryu-blueprint`.
...
Read an agent's plan as a **graph plus rendered markdown**, mark it up, and send back a
verdict the agent can act on.

An agent's plan arrives as a wall of prose because prose is what a model writes. Prose
is a sequence; the work almost never is. Blueprint takes the same plan and shows the
two things the wall hides:

```
  markdown ──▶ blocks (heading/para/list/code/quote/mermaid)   ← annotate any one
  steps[]  ──▶ dependency graph from depends_on                ← see what blocks what
```

The graph is not decoration. It is the answer to "what is everything else waiting
on", and it is derived — nobody draws it, it falls out of the `depends_on` edges the
agent already had to declare. A step with five inbound edges is the step to argue
about; in a numbered list it looks like every other line.

## The loop

1. **Publish.** The agent calls `blueprint__plan_publish` with a title, the plan
...
5. **Read back.** The agent polls `blueprint__plan_status`, which returns the verdict
...
7. **Execute.** `blueprint__step_update` moves a step through
...
| Tool | |
|---|---|
| `blueprint__plan_publish` | Publish or revise. `plan_id` present → new revision of that plan; absent → new plan. Returns `{ plan_id, revision, status, review_url }`. |
| `blueprint__plan_status` | `{ status, verdict?, feedback, annotation_count, revision }`. `wait_secs` clamps to 0..=60 and polls internally; **on timeout it returns `in_review`**, which is neither approval nor rejection. |
| `blueprint__plan_get` | The whole plan: current revision, annotations, verdict. |
| `blueprint__step_update` | Set a step's status as the work lands. |
...
- `CORE_PLUGINS` in `apps/core/src/plugins/builtins.rs` — `BLUEPRINT_PLUGIN_ID`.
  **Nothing enforces this one.** Core tier is what lets the sidecar spawn at all:
  ...
  never starts, `blueprint__plan_publish` does not exist, and the output style becomes
